...
public:
    MIN_DESCRIPTION {"Process audio with a Python class. Attributes and messages are generated from the class's type-annotated members, and the class's process() method runs on the audio signal. The source file is watched and hot-reloaded on save."};
    ...
    void update_source() {
        ...
        if (m_module) {
            ...
            if (!m_module) {
                PyErr_Print();
                cerr << "Failed to load module '" << m_python_source << "' (searched " << m_scripts_dir.string() << ")" << endl;
                return;
            }
        }
...
private:
    ...
    void bind_process(PyObject* fn, PyObject* hints) {
        ...
        if (in_count > 1)
            cerr << "process() declares " << in_count << " inputs but only the first is supported (single-channel object)" << endl;
        if (returns_tuple) {
